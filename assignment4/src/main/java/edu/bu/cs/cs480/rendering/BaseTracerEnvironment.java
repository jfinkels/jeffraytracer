...
public class BaseTracerEnvironment implements TracerEnvironment {
  ...
  protected Vector3D[] traceAllRays(final Ray[] rays1D) {
    final Vector3D[] result = new Vector3D[rays1D.length];
    for (int i = 0; i < rays1D.length; ++i) {
      result[i] = this.trace(rays1D[i], 1);
    }
    return result;
  }
  ...
  public RenderedImage render() {
    ...
    final Vector3D[] pixels = this.traceAllRays(rays1D);
  ...
  protected int[] postProcessing(final Vector3D[] pixels) {
    ...
    for (int i = 0; i < pixels.length; ++i) {
      result[i] = DoubleColor.toRGB(pixels[i]);
    }
    return result;
  }
   ...
   */
  @Override
  public Vector3D trace(final Ray ray, final int depth) {
