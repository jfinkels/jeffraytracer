...
public class PA1 extends JFrame implements GLEventListener, KeyListener,
  ...
  /** The message which displays whether the polygon is convex or concave. */
  private String concavityMessage = "";
  ...
  public void display(final GLAutoDrawable drawable) {
    ...
    // render the polygon
    PolygonDrawer.draw(drawable, this.polygon);

    // display a message for concavity/convexity
    if (this.polygon.isConcave()) {
      this.concavityMessage = "Polygon is concave";
    } else {
      this.concavityMessage = "Polygon is convex";
    }

    drawString(drawable, 20, this.canvas.getHeight() - 60,
        this.concavityMessage, GLUT.BITMAP_TIMES_ROMAN_24);

    if (this.insideOutsideMessage != null)
      drawString(drawable, 20, this.canvas.getHeight() - 20,
          this.insideOutsideMessage, GLUT.BITMAP_TIMES_ROMAN_24);
  }
  ...
  public void keyTyped(final KeyEvent key) {
    ...
    case 'T':
    case 't':
      this.polygon = this.testCases.next();
      break;
  ...
  public void mouseDragged(final MouseEvent mouse) {
    if (this.dragging) {
      this.polygon.moveVert(mouse.getX(), mouse.getY());
    }
  }
  ...
  public void mousePressed(final MouseEvent mouse) {
    ...
    if (button == MouseEvent.BUTTON1)
      this.polygon.addVert(mouse.getX(), mouse.getY());

    // on middle clicks, display whether the point under the mouse is inside or
    // outside the polygon
    else if (button == MouseEvent.BUTTON2) {
      if (this.polygon.isInside(mouse.getX(), mouse.getY()))
    ...
    // on right clicks, select the closest vertex
      ...
      int x = mouse.getX();
      int y = mouse.getY();
      this.polygon.selectVert(x, y);
      this.polygon.moveVert(x, y);
      this.dragging = true;
    }
